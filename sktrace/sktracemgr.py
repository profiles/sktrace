            ...
            for i in range(31):
                if pre_ctx.general_regs[i] != ctx.general_regs[i]:
                    ...
                    if print_every_change:
                        if eveny_change_str:
                            eveny_change_str += (
                                ", x{}={}->{}".format(str(i), pre_ctx.general_regs[i], ctx.general_regs[i]))
                        else:
                            eveny_change_str = (
                                "{}\t; x{}={}->{}".format(self.simple_str(), str(i), pre_ctx.general_regs[i],
                                                          ctx.general_regs[i]))
...
class Arm64Ctx:
    def __init__xxx(self, pc, sp,
        ...
        self.pc = pc
        self.sp = sp
        self.general_regs = [int(x0, 16), int(x1, 16), int(x2, 16), int(x3, 16), int(x4, 16), int(x5, 16), int(x6, 16),
                             int(x7, 16), int(x8, 16), int(x9, 16),
                             int(x10, 16), int(x11, 16), int(x12, 16), int(x13, 16), int(x14, 16), int(x15, 16),
                             int(x16, 16), int(x17, 16), int(x18, 16), int(x19, 16),
                             int(x20, 16), int(x21, 16), int(x22, 16), int(x23, 16), int(x24, 16), int(x25, 16),
                             int(x26, 16), int(x27, 16), int(x28, 16),
                             int(fp, 16), int(lr, 16)]
    ...
    def on_message(self, payload):
        ...
        if type == 'inst':
            ...
            ctx = Arm64Ctx(val["pc"], val["sp"],
                           val["x0"], val["x1"], val["x2"], val["x3"], val["x4"], val["x5"], val["x6"], val["x7"],
                           val["x8"], val["x9"],
                           val["x10"], val["x11"], val["x12"], val["x13"], val["x14"], val["x15"], val["x16"],
                           val["x17"], val["x18"], val["x19"],
                           val["x20"], val["x21"], val["x22"], val["x23"], val["x24"], val["x25"], val["x26"],
                           val["x27"], val["x28"],
                           val["fp"], val["lr"])
